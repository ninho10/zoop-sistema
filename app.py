...
def add_product():
    ...
    if request.method == 'POST':
        ...
        for photo_field in ['photo', 'photo2', 'photo3']:
            print(f"DEPURAÇÃO: Verificando {photo_field} na requisição: {photo_field in request.files}")
            if photo_field in request.files:
                file = request.files[photo_field]
                print(f"DEPURAÇÃO: Nome do arquivo ({photo_field}): {file.filename}")
                
                if file and file.filename != '':
                    if allowed_file(file.filename):
                        filename = secure_filename(file.filename)
                        save_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
                        print(f"DEPURAÇÃO: Salvando arquivo {photo_field} em: {save_path}")
                        try:
                            file.save(save_path)
                            data_to_insert[photo_field] = filename
                            print(f"DEPURAÇÃO: Arquivo {photo_field} salvo com sucesso")
                        except Exception as e:
                            print(f"DEPURAÇÃO: Erro ao salvar arquivo {photo_field}: {e}")
                            flash(f'Erro ao salvar imagem {photo_field}: {e}', 'warning')
                    else:
                        print(f"DEPURAÇÃO: Arquivo não permitido. Extensões: {ALLOWED_EXTENSIONS}")
                        flash(f'Imagem {photo_field} não salva. Tipo de arquivo não permitido.', 'warning')
